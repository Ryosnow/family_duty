...
- [x] 按日期、成员和任务名称筛选历史记录。
- [x] 查看任务详情、完成者、完成时间和调整原因。
- [x] 查看全部历史任务，而不只显示首页最近 8 条记录或当天任务。
- [x] 从历史记录重新创建类似任务。

### 计划工作量与实际完成量

- [x] 展示每位成员本周被分配的任务数和计划分值。
- [ ] 同时展示已完成、待完成和逾期数量。
...
- [ ] 首次引导支持一次添加多个成员，而不是只创建一名成员。
- [ ] 支持成员颜色或头像，增强共享 iPad 上的识别度。
- [ ] 任务卡增加带确认步骤的快速完成。
- [ ] 首页增加今日待完成分值和逾期数量。
- [ ] 支持按任务截止日发送提醒，而不仅是每日汇总。
- [x] 设置页增加数据管理入口，集中提供备份和恢复。
